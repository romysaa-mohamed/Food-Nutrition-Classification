[REPORT.md](https://github.com/user-attachments/files/31270984/REPORT.md)
...
- **Missing value heatmap** to visualize the pattern of missing data across the dataset.
- **Distribution of `health_score`** via histogram + KDE, and boxplot to detect outliers. The score distribution is concentrated between 40 and 60, with a peak around 50 (13,914 rows at exactly 50).
- **Distribution of each nutritional feature**, capped at the 99th percentile to reduce the visual impact of extreme outliers (e.g. `sodium_mg` had a max of ~4.69 million mg, clearly an extreme outlier relative to a 75th percentile of ~481 mg).
- **Boxplots** for every nutritional feature to identify outliers.
- **Correlation matrix** between all nutritional features and `health_score`.
- **Ranked bar chart** of feature correlations with `health_score`.
- **Scatter plots** of each nutritional feature against `health_score` (capped at the 99th percentile).
- **Categorical analysis:**
  ...
  - `health_score` distribution across the top 10 food categories and across food types (boxplots).
...
- `health_score` varies noticeably across food categories, confirming `food_category` carries useful predictive signal.
- Several nutritional columns (`sodium_mg`, `iron_mg`, `fat_g`, etc.) contain extreme outliers that required capping before modeling.
...
**Feature importance** was extracted from the Decision Tree to identify which nutritional features drive predictions the most (top 15 features visualized as a horizontal bar chart in the notebook).
