[README (1).md](https://github.com/user-attachments/files/31259527/README.1.md)
<div align="center">
...
> **⚠️ Note on the near-perfect Decision Tree score:** A 99.89% accuracy (with 100% training accuracy) is unusually high and is very likely because `health_class` is a deterministic, rule-based transformation of `health_score`, and `health_score` itself appears to be a formula derived from the same nutritional features used as inputs — making the classification boundary almost perfectly learnable by a tree. This is called out here for transparency; a natural next step is to check how `health_score` was originally computed and consider whether this setup constitutes data leakage for a "real-world" deployment scenario. Random Forest and KNN, which don't overfit deterministic thresholds as easily, are more representative of performance you'd expect on genuinely noisy, real-world nutrition data.
...
- Investigate and document exactly how `health_score` is computed, to confirm/rule out data leakage.
- Try gradient-boosting models (XGBoost, LightGBM, CatBoost) for comparison on a leakage-free target.
